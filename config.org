...
#+begin_src emacs-lisp :tangle yes
    ...
    ;; Like C-M-j find a simalar binding for buffer switcing it is simply to good
      ...
      ;; process
      (mk/leader-keys
	"p" '(:ignore t :which-key "launch programs" )
	"pf" '(mk/launch-firefox  :which-key "firefox" )
	"pe" '(mk/launch-epiphany :which-key "epiphany" ))

      (mk/leader-keys
	"o" '(:ignore t :which-key "buffer managment" )
	"oi" '(insert-buffer :which-key "insert buffer" )
	"oe" '(switch-to-buffer :which-key "switch buffers" ))

      (mk/leader-keys
	"e" '(:ignore t :which-key "elsip evaluations" )
	"ep" '(eval-last-sexp :which-key "eval at point" )
	"eo" '(eval-expression  :which-key "eval expression" )
...
#+begin_src emacs-lisp :tangle yes
    ...
    (defhydra hydra-text-scale (:timeout 4)
      ...
      ("r" text-scale-decrease "out")
      ("s" text-scale-set "Equalize")
      ("t" nil "finished" :exit t))
...
;;  <"h" shrink-window-horizontally "out">
    ...
    (mk/leader-keys
      "ea" '(vterm-toggle :which-key "vterm popper"))
